...
export class TweetComponent implements OnInit {
  ...
  AddTweet() {
    ...
    if (this.isEdit) {
      this.httpc.put("https://tweetwebapi20220725021826.azurewebsites.net/api/Tweet", admindto).subscribe(res => this.PostSuccess(res), res => this.PostError(res));
    }
    else {
    let filetoUpload=<File>this.files[0];
    const formData=new FormData();
    formData.append('file',filetoUpload,filetoUpload.name)
    this.httpc.post("https://tweetwebapi20220725021826.azurewebsites.net/api/Upload",formData).subscribe(res=>{console.log(res); this.img=res;admindto.tweetImage=this.img.imageUrl;this.AddTweetData(admindto);},res=>console.log(res));
  ...
  AddTweetData(admindto:any){
    this.httpc.post("https://tweetwebapi20220725021826.azurewebsites.net/api/Tweet", admindto).subscribe(res => this.PostSuccess(res), res => this.PostError(res));
  }
  PostSuccess(res: any) {
    console.log(res);
  ...
  getData(email: any) {
    console.log("Hi");
    this.httpc.get("https://tweetwebapi20220725021826.azurewebsites.net/api/Tweet/GetTweetsByUserName?username=" + email).subscribe(res => this.GetSuccess(res), res => this.GetError(res));
  }
